...
def main():
    ...
    if config_path.exists():
        with open(config_path, 'r') as f:
            config = json.load(f).get("process_personas_4chan", {})
    else:
        config = {}

    parser = argparse.ArgumentParser(description="Process personas: embed, deduplicate, select")
    parser.add_argument("--input", default=config.get("input", "data/output/personas.ndjson"), help="Input JSONL file")
    ...
    final_output = output_dir / f"persona_selected_{args.n_select}.jsonl"
